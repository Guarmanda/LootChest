...
public class Fallv_1_13_2 implements IFallPacket {
     ...
     */
    public Fallv_1_13_2(Location loc, Material headItem, int height, double speed, JavaPlugin plugin) {
      ...
      */
      private static ItemStack getNmsItemStackFromMaterial(Material material) {
        ...
        for(Item item : Arrays.stream(Blocks.class.getFields()).map(field -> {
            try {
                return ((Block)field.get(null)).getItem();
            } catch (IllegalArgumentException | IllegalAccessException e) {
                //System.out.println("Error while getting item to set armorstand head");
            }
            return null;
        }).toArray(Item[]::new)) {
            if (item == null) {
